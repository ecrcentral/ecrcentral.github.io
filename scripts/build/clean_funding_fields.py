#!/usr/bin/env python3
"""
Clean up legacy fields in all funding YAML files.

For each data/entries/fundings/*.yaml:
...
Usage:
    python scripts/build/clean_funding_fields.py
    python scripts/build/clean_funding_fields.py --dry-run
"""
import argparse
import re
import sys
...
FUNDINGS_DIR = REPO_ROOT / 'data' / 'entries' / 'fundings'
...
def main():
    parser = argparse.ArgumentParser(description='Clean legacy fields from funding YAML files.')
    parser.add_argument('--dry-run', action='store_true', help='Preview without writing')
    args = parser.parse_args()

    if not FUNDINGS_DIR.exists():
        print(f'[ERR] directory not found: {FUNDINGS_DIR}', file=sys.stderr)
        sys.exit(1)

    files = sorted(FUNDINGS_DIR.glob('*.yaml'))
    print(f'Processing {len(files)} files in {FUNDINGS_DIR.name}/')
